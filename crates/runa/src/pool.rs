...
use runa_engine::{GenEvent, GenerateRequest, LoadConfig, Placement, load};
use runa_fit::{
...
impl ModelPool {
    ...
    /// Register one more model file under a stem id (daemon on-demand
    /// serving of paths it was not started with). No-op when the id or
    /// an equal path is already known. Returns the id.
