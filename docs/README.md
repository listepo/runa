...
| [`release.md`](release.md) | How a release runs: the two entry points (`bump.yml`, `release-plz.yml`), `scripts/release.sh`, the `verify` gate, artifacts, and what is deliberately not wired |
| [`../CHANGELOG.md`](../CHANGELOG.md) | Generated by git-cliff from commit subjects (`cliff.toml`); never edited by hand |
